...
class DataSource {
    ...
    private object Constants {
        ...
        val lowWaitTimeRange: ClosedRange<Double> = 0.0..0.7 // lower bound must be >= 0.0, upper bound must be > lower bound
        val highWaitTimeRange: ClosedRange<Double> = 1.0..1.3 // lower bound must be >= 0.0, upper bound must be > lower bound
    ...
    suspend fun fetch(next: String?, completionHandler: FetchCompletionHandler) {
    ...
    private fun processRequest(next: String?): ProcessResult {
        ...
        if (isError) {
            ...
            if (next != null && (nextIntValue == null || nextIntValue < 0)) {
                ...
                var fetchedPeople: ArrayList<Person> = ArrayList(people.subList(beginIndex, endIndex))
